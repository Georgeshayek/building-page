    ...
    return(
        ...
        <div className="text-center my-6">
            ...
            <select value={cat} onChange={categoryHandlerChang}>
             {category.map(mov=><option value={mov}>{mov}</option>)}
      </select>
        <div className="flex justify-around max-lg:flex-col max-lg:items-center ">
            {filterImage.map((img,i)=><div key={i} className="flex flex-col"><Image className="object-cover mx-4 rounded max-lg:m-2 h-[250px] w-[350px]" width={250} height={200} src={img.imageurl} alt={img.title} /><h4>{img.title}</h4></div>)}
